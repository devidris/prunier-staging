...
<img style="border:5px solid #dabf664f;border-radius:500px;width:400px;clip-path:ellipse(50% 50%);" 
     src="{if is_array($product.dwf_zoom_grain) && isset($product.dwf_zoom_grain[0])}{$product.dwf_zoom_grain[0]}{elseif !empty($product.dwf_zoom_grain)}{$product.dwf_zoom_grain}{/if}" 
     alt="{$product.name}"> 
  ...
  <div class="mt-2 mb-3">
    <p class="mb-0">{l s='Livraison réfrigérée ❄️ en <strong>24 à 48h</strong> (jours ouvrés) en <strong>France</strong>' d='Shop.Theme.CreathemeCatalog'}</p>
    <p class="mt-1" style="color: #27B7B1;"><strong>{l s='Profitez de la livraison offerte dès 130€ en France métropolitaine.' d='Shop.Theme.CreathemeCatalog'}</strong></p>
  </div>
{/if}
